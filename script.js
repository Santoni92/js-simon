...
const arrayNumeriDaIndovinare = creaArrayNumeriDaIndovinare(); //array in cui memorizzare i numeri generati random e che l'utente dovrà indovinare
...
{
    const arrayNumeriUtente = creaArrayNumeriUtente();   //inizializzo l'array che sarà destinato a contenere i cinque numeri inseriti dall'utente
...
function creaArrayNumeriDaIndovinare()
{
    const arrayNumeriDaIndovinare = [];
    for(let i = 0; i < 5; i++)
    {
   ...
   return arrayNumeriDaIndovinare;
}

function creaArrayNumeriUtente()
{
    const arrayNumeriUtente = [];
    for(let i = 0; i < 5; i++)
    {
        let numeroInseritoDallUtente = parseInt(prompt('Inserisci numero: '));
        numeroInseritoDallUtente = controllaNumeroInserito(numeroInseritoDallUtente);
...
function controllaNumeroInserito(numeroInserito)
{
    while(isNaN(numeroInserito))  //rifaccio la richiesta di inserimento numero da parte dell'utente finché quest'ultimo non inserisce un numero
